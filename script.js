...
function bindReader(){
 ...
 const view=$('#paper-viewport');view?.addEventListener('touchstart',e=>{if(e.touches.length===1)drag={x:e.touches[0].clientX,y:e.touches[0].clientY}}, {passive:true});view?.addEventListener('touchend',e=>{if(!drag)return;const dx=e.changedTouches[0].clientX-drag.x,dy=e.changedTouches[0].clientY-drag.y;drag=null;if(Math.abs(dx)>70&&Math.abs(dx)>Math.abs(dy)*1.35){selectPage(currentPage+(dx<0?1:-1))}}, {passive:true});
}
